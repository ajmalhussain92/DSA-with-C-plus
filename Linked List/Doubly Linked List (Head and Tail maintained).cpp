...
void push_back (Node*& head, Node*& tail, int data) {
    ...
    tail->next = ptr;           // Link tail node to new node
    ptr->prev = tail;           // Link new node back to tail node
    tail = ptr;                 // Make the last new node as tail
}

// Insert @ Middle
void push_middle (Node*& head, Node*& tail, int data) {
    Node* ptr = new Node ();    // Insert node (Node constructor initializes prev and next to nullptr)
    ptr->data = data;
    ...
    if (ptr->next != nullptr) {
        ...
        tail = ptr;             // Update tail if inserted at the end
    }
}
...
void pop_front (Node*& head, Node*& tail) {
    ...
    delete temp;
    del = nullptr;              // Not required, but good habit
    
    if (head != nullptr) head->prev = nullptr;
}

// Delete @ Middle
void pop_middle (Node*& head, Node*& tail) {
    if (head == nullptr) {      // if empty list, simply return
        cout << "Empty list! Can't delete.\n";
        return;
    }
